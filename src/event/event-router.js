...
EventRouter
  ...
  .delete('/', jsonBodyParser, async (req, res, next) => {
    ...
    if (type === 'meal') {
      ...
      const symptom = await EventService.getSymptomById(req.app.get('db'), id)
      if(!symptom){
        return res.status(404).send({error:'Symptom not found'})
      }
      if(symptom.user_id!==req.user.id){
        return res.status(403).send({error:'Symptom does not belong to user'})
      }
